...
const KP_TYPE = new URLSearchParams(window.location.search).get('type') ?? 'kks-kp-2'
...
const renderInsertPdf = async () => {
  ...
  try {
    const pdf = await getDocument({
      url: new URL('../../assets/pdf/add-v2.pdf', import.meta.url).href,
      verbosity: 0
    }).promise
    ...
    frame.src = `${new URL('../../assets/pdf/add-v2.pdf', import.meta.url).href}#toolbar=0&navpanes=0&scrollbar=0`
